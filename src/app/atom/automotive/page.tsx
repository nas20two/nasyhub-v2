...
import { ArrowRight, Loader2, CheckCircle, Car , Upload } from "lucide-react";
import Link from "next/link";

const STORAGE_KEY = "atom-a…form";
...
const businessTypes = ["Car Dealership", "Auto Detailer", "Mechanic / Workshop", "Panel Beater", "Car Wash", "Tyre Shop", "Auto Accessories", "Truck Dealer", "Motorcycle Dealer", "Car Rental", "Other"];
const styles = ["Sleek & Premium", "Professional & Trusted", "Fast & Action", "Family Friendly", "Performance & Sport", "Budget Friendly"];
...
export default function AutomotivePage() {
  ...
  useEffect(() => {
    document.title = "Automotive & Dealership Template | Atom";
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/atom-submit", {
        ...
        body: JSON.stringify({ template: "automotive", data: form, email: form.email, tier: "basic" }),
      });
      ...
      <section className="relative overflow-hidden">
        ...
        <nav className="max-w-3xl mx-auto px-4">
          <div className="flex items-center justify-between py-4">
            ...
            <span className="text-xs text-muted-foreground">Automotive</span>
          </div>
        </nav>
        <div className="max-w-3xl mx-auto px-4 pt-8 pb-12 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-foreground">
            Automotive &amp; Dealership <span className="atom-gradient-text">Promo</span>
          </h1>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto mb-3">
            A 60-second showcase for your dealership, workshop, or auto service business.
            Inventory, quality, and your best offer — all AI-generated.
          </p>
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mb-3">Showcase your inventory, service bay, and facilities. 4-6 photos recommended.</p>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input type="text" value={form.businessName} onChange={(e) => updateField("businessName", e.target.value)}
                      placeholder="Sydney Prestige Motors" className={inputClass} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="text" value={form.speciality} onChange={(e) => updateField("speciality", e.target.value)}
                      placeholder="Luxury imports, Euro servicing, ceramic coating" className={inputClass} />
                  </div>
                  <div>
                    <label className={labelClass}>Location</label>
                    <input type="text" value={form.location} onChange={(e) => updateField("location", e.target.value)}
                      placeholder="Alexandria, Sydney" className={inputClass} />
                  </div>
                  <div>
              ...
              <div className="p-6 rounded-2xl border border-border bg-card/80 backdrop-blur-sm space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className={labelClass}>Highlight 1</label>
                    <input type="text" value={form.highlight1} onChange={(e) => updateField("highlight1", e.target.value)}
                      placeholder="100+ vehicles in stock with CarHistory reports" className={inputClass} />
                  </div>
                  <div>
                    <label className={labelClass}>Highlight 2</label>
                    <input type="text" value={form.highlight2} onChange={(e) => updateField("highlight2", e.target.value)}
                      placeholder="Factory-trained technicians, 12-month warranty" className={inputClass} />
                  </div>
                  <div>
                    <label className={labelClass}>Highlight 3</label>
                    <input type="text" value={form.highlight3} onChange={(e) => updateField("highlight3", e.target.value)}
                      placeholder="Free 7-day money-back guarantee on all used stock" className={inputClass} />
                  </div>
                </div>
              </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input type="email" value={form.email} onChange={(e) => updateField("email", e.target.value)}
                      placeholder="sales@sydneyprestige.com.au" className={inputClass} required />
                  </div>
                </div>
              </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="url" value={form.ctaLink} onChange={(e) => updateField("ctaLink", e.target.value)}
                      placeholder="https://sydneyprestige.com.au/stock" className={inputClass} />
                  </div>
                </div>
              </div>
              ...
              <div className="text-center pt-2">
                ...
                >
                  {submitting ? <><Loader2 className="w-5 h-5 animate-spin" /> Generating your video...</>
                    : <><Car className="w-5 h-5" /> Generate Automotive Video</>}
                </button>
              </div>
            </form>
          </motion.div>
